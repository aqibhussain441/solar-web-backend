<div x-data x-on:refresh-product-type-list.window="$wire.set('deleteModelId', null);$wire.$refresh();">
    <div class="flex justify-between items-center py-4">
        <h2 class="text-2xl">Product Types</h2>
        {{-- Search filter start --}}
        <div class="relative w-64">
            <x-input type="text" placeholder="Search..." wire:model.live="searchQuery" />
        </div>
        <livewire:admin.types.save-type />
    </div>
        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                ...
                @foreach ($this->types as $type)
                    <tr wire:key='{{ $type->id }}' class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        <th scope="row"
                            class="p-2 text-center font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {{ $type->id }}
                        </th>
                        <td class="p-2">
                            {{ Str::words($type->name, 5, '...') }}
                        </td>
                        <td class="p-1">
                            {{ $type->slug }}
                        </td>
                        ...
                        <td class="p-2">
                            <x-button type="button"
                                @click="$dispatch('edit-product-type', { id: {{ $type->id }} })"
                                wire:loading.attr="disabled">
                                {{ __('Edit') }}
                            </x-button>
                            <x-danger-button type="button" wire:click="$set('deleteModelId', {{ $type->id }})"
                                wire:loading.attr="disabled">
                                {{ __('Delete') }}
                            </x-danger-button>
    ...
    <x-dialog-modal wire:model.live="deleteModelId">
        ...
        <x-slot name="footer">
            ...
            <x-danger-button class="ms-3" wire:click="$dispatch('delete-product-type', { id: {{ $deleteModelId }} })"
                wire:loading.attr="disabled">
                {{ __('Delete') }}
            </x-danger-button>
        </x-slot>
